...
ADD .bashrc ~/.bashrc
RUN apk --no-cache add -y build-essential python3-dev mono-complete golang nodejs default-jdk npm wget git ctags\
    && apk --no-cache add libtinfo-dev locales cmake make gcc g++ bash tzdata -y \
...
RUN git clone https://github.com/VundleVim/Vundle.vim.git ~/.vim/bundle/Vundle.vim \
    ...
    && apk del libtinfo-dev build-essential cmake wget make gcc g++ tzdata -y \
